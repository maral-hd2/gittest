...
for tick in range(len(date)):
	...
	for i_ticker in range(len(tickers)):
		dates  = "https://api.intrinio.com/fundamentals/reported?identifier={}&statement=income_statement&type=QTR&date=2018/11/01&page_size=8&hide_paging=true&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker])
		x = get_data(dates)
		for j in range(len(x["data"][:])):
			filing_date = x["data"][:][j]["filing_date"].split("T")[0]
			# print(i)
			...
			dates_after = pandas.bdate_range(filing_date, periods=14, freq='B').format()

			dates_before_period_end = dates_before[-1]
			dates_before_period_start = dates_before[0]
			dates_after_period_end = dates_after[-1]
			dates_after_period_start = dates_after[0]
	
			try:
			
				open_prices_before = "https://api.intrinio.com/historical_data?identifier={}&item=open_price&start_date={}&end_date={}&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker], dates_before_period_start, dates_before_period_end)			
				close_prices_before = "https://api.intrinio.com/historical_data?identifier={}&item=close_price&start_date={}&end_date={}&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker], dates_before_period_start, dates_before_period_end)
				close_prices_after = "https://api.intrinio.com/historical_data?identifier={}&item=close_price&start_date={}&end_date={}&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker], dates_after_period_start, dates_after_period_end)
				high_prices  = "https://api.intrinio.com/historical_data?identifier={}&item=high_price&start_date={}&end_date={}&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker], dates_before_period_start, dates_before_period_end)
				low_prices  = "https://api.intrinio.com/historical_data?identifier={}&item=low_price&start_date={}&end_date={}&api_key=OjljZTNjZDdlNDIyOGRiYTQ2MTJkZDhkNTI4MjVhOTI2".format(tickers[i_ticker], dates_before_period_start, dates_before_period_end)
				...
				for k in range(len(close_prices_before["data"][:])):
					...
					# close_dataframe = pandas.DataFrame({})
						ticker.append(tickers[i_ticker])
						...
						MOM = df["Momentum_7"].dropna().tolist()[-5:]
						MOM = np.array(MOM)
						...
						MOM_1.append(df["Momentum_7"].dropna().tolist()[-1])
						MOM_2.append( df["Momentum_7"].dropna().tolist()[-2])
						MOM_3.append(df["Momentum_7"].dropna().tolist()[-3])
						MOM_4.append( df["Momentum_7"].dropna().tolist()[-4])
						MOM_5.append(df["Momentum_7"].dropna().tolist()[-5])
...
print(len(ticker), len(ROC_1))


ticker_final = pandas.DataFrame({"Ticker" : ticker})

ticker_final.to_csv("final_tickers.csv") 


final_features = pandas.DataFrame({"ROC" : ROC_signal_con, "ROC_1" : ROC_1,  "ROC_2" : ROC_2, "ROC_3" : ROC_3,  "ROC_4" : ROC_4, "ROC_5" : ROC_5, "RSI" : RSI_signal_con,  "RSI_1" : RSI_1,  "RSI_2" : RSI_2, "RSI_3" : RSI_3,  "RSI_4" : RSI_4, "RSI_5" : RSI_5, "MOM" : MOM_signal_con, "MOM_1" : MOM_1,  "MOM_2" : MOM_2, "MOM_3" : MOM_3,  "MOM_4" : MOM_4, "MOM_5" : MOM_5, "ADX" : ADX_signal_con, "ADX_1" : ADX_1,  "ADX_2" : ADX_2, "ADX_3" : ADX_3,  "ADX_4" : ADX_4, "ADX_5" : ADX_5, "MACD" : MACD_signal_con, "MACD_1" : MACD_1,  "MACD_2" : MACD_2, "MACD_3" : MACD_3, "After" : after_signal_con})
